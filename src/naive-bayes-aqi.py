...
# Model
gau = GaussianNB()
gau_mod = gau.fit(X_train, y_train)
gau_preds = gau_mod.predict(X_test)
# print(f"Gau Probs:\n{gau_mod.predict_proba(X_test).round(3)}\n")
print(f"{'-'*10}Validation{'-'*10}\n{gau_mod.score(X_test, y_test)}")
gau_cm = confusion_matrix(y_true=y_test, y_pred=gau_preds)
